...
/*
 * This is a patched version of the driver from the linux
 * tree. This includes portions from this patch as well
 * to enable the strobe line:
 * https://github.com/raspberrypi/linux/pull/4621/files
 * 
...
enum imx_477_strobe {
	IMX477_STROBE_OFF        = 0,
	IMX477_STROBE_SINGLE     = 1,
	IMX477_STROBE_CONTINUOUS = 2,
};
...
static int imx477_open(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
    ...
    printk(KERN_ERR "Opened Exclosure imx477 firmware");
					    ...
					    unsigned int fstrobe_width)
{
    ...
    printk(KERN_ERR "fstrobe_width %d", fstrobe_width);
	if (ret != 0) {
    ...
    printk(KERN_ERR "fstrobe_mult %d", fstrobe_mult);
	if (ret != 0) {
...
static int imx477_start_streaming(struct imx477 *imx477)
{
    ...
    /* Enable the strobe line for a nice 1ms pulse */
    imx477_set_call_for_strobe_mode(imx477, IMX477_STROBE_CONTINUOUS);
    imx477_set_call_for_strobe_delay(imx477, 0);
    imx477_set_call_for_strobe_width(imx477, 1000);
